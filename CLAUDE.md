...
```
pnpm capture && pnpm catalog && pnpm emit-as-published
(cd ../sailscoring && pnpm archive-generate ../ksc-archive/as-published.config.json)
pnpm identities
```
...
5. **Never edit `identities.json` by hand, and never change a slug.** It is
   ...
   When two spellings might be one person, merge only on the evidence tests in
   CLARIFICATIONS.md §9; over-linking puts one sailor's results in another's
   record, which is worse than leaving them apart and far harder to notice.

6. **Never rename an emitted series `key`.** Series ids are deterministic
...
7. **Record every judgement call in [`CLARIFICATIONS.md`](CLARIFICATIONS.md).**
...
8. **Derived fields carry their provenance.** `sources/catalog.json` records,
